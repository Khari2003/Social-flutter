...
const updateUser = async (req, res) => {
  if (req.user._id === req.params.id || req.user.role === "admin") {
    ...
    try {
      ...
      // Xử lý tọa độ theo schema mới
      if (lat !== undefined || lng !== undefined) {
        // Validate và xử lý latitude
        if (lat !== undefined) {
          ...
          updateData["coordinates.lat"] = latitude;
        }

        // Validate và xử lý longitude
        if (lng !== undefined) {
          ...
          updateData["coordinates.lng"] = longitude;
        }
      }
